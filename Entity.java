//COMP 1006A/1406A Assignment 5 Problem 1
//By Alexander Kuhn, ID# 101023154, August 9th, 2016
//Purpose: This class exists to be inherited, but I've had to alter it a little
//For Problem 1, I changed it so it properly registers when something leaves the borders of the map (with a little leeway) and throws an exception
//Aside from that, I've made it so that the setters for speedX/speedY are no longer final
//I did this because I'm assuming testing code will use those setters, and for them to work properly for some of my classes, I'll need to override them
/**
...
public abstract class Entity{
 ...
 /** maximum speed for a player for use in tournament */
  ...
  public final void updatePosition(long time, Field field, int id) throws EntityOutOfBoundsException{
    ...
    if ((((this.x + (time * this.speedX) / TIME_SCALE) > 820) || (this.x + (time * this.speedX) / TIME_SCALE) < 0) || (((this.y + (time * this.speedY) / TIME_SCALE) > 820) || (this.y + (time * this.speedY) / TIME_SCALE) < 0)) {
  ...
  /** sets the speed in the x-direction of this entity 
    ...
    * @param id should be the entity's id
    */
  protected void setSpeedX(double speedX, int id){ 
    /* check if the caller knows this entity's id */
      if( id != this.id ){
  ...
  /** sets the current speed in the y-direction of this entity 
    ...
    * @param id should be the entity's id
    */
  protected void setSpeedY(double speedY, int id){ 
    /* check if the caller knows this entity's id */
    if( id != this.id ){
